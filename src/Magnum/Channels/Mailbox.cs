...
namespace Magnum.Channels
{
		...
		Channel<T>
	{
		...
		void Receive(ConsumerFactory<T> consumer);
		...
		void Receive(ConsumerFactory<T> consumer, TimeSpan timeout, Action timeoutCallback);
		...
		/// <param name = "timeout">The time period to wait for a message</param>
		/// <param name = "timeoutCallback">The method to call if a message is not received within the timeout period</param>
		void Receive(ConsumerFactory<T> consumer, int timeout, Action timeoutCallback);
	}
}