...
namespace Stact.ServerFramework.Internal
{
	...
	public class BlockWriterChannel :
		MessageChannel
	{
		const int PaddingLength = 8;
		readonly BlockWriter _blockWriter;
		readonly IDictionary<string, string> _noHeaders;
		...
		public BlockWriterChannel(BlockWriter blockWriter, Serializer serializer)
		{
			_noHeaders = new Dictionary<string, string>();
		...
		public void Send<T>(T message, IDictionary<string, string> headers)
		{
			...
			using (var ts = new StreamWriter(ms.PreventClose()))
			{
				...
				_blockWriter.Write(arraySegment, _unsentCallback);
			}
		}
