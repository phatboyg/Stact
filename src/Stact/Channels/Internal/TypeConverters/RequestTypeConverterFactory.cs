...
namespace Stact.Internal.TypeConverters
{
	...
	public class RequestTypeConverterFactory<T> :
		HeaderTypeConverterFactory<T>
	{
		...
		public RequestTypeConverterFactory()
		{
			_convert = GenerateConvertMethod();

			_ignore = !typeof(T).IsGenericType || !typeof(T).Implements(typeof(Request<>));
			if (!_ignore)
				_requestType = typeof(T).GetGenericTypeDeclarations(typeof(Request<>)).Single();
		}
		...
		public bool CanConvert<TInput>(TInput input, out Func<object, T> converter)
		{
			...
			if (typeof(TInput).Implements(typeof(Request<>)))
			{
				Type messageType = typeof(TInput).GetGenericTypeDeclarations(typeof(Request<>)).Single();
